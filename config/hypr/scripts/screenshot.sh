...
option_capture_1="Todas las pantallas"
option_capture_2="Pantalla actual"
option_capture_3="Selección"
...
copy='Copiar'
save='Guardar'
copy_save='Copiar y guardar'
edit='Editar'

# Rofi CMD
rofi_cmd() {
    rofi -dmenu -replace -config ~/.config/rofi/config-screenshot.rasi -i -no-show-icons -l 2 -width 30 -p "Captura"
}
...
# CMD
timer_cmd() {
    rofi -dmenu -replace -config ~/.config/rofi/config-screenshot.rasi -i -no-show-icons -l 5 -width 30 -p "Demora"
}
...
# CMD
type_screenshot_cmd() {
    rofi -dmenu -replace -config ~/.config/rofi/config-screenshot.rasi -i -no-show-icons -l 3 -width 30 -p "Qué capturar"
}
...
copy_save_editor_cmd() {
    rofi -dmenu -replace -config ~/.config/rofi/config-screenshot.rasi -i -no-show-icons -l 4 -width 30 -p "Qué hacer"
}
...
timer() {
    if [[ $countdown -gt 10 ]]; then
        notify_user \
            --a "${APP_NAME}" \
            --i "${NOTIFICATION_ICON}" \
            --s "Captura en ${countdown} segundos" \
            --m "" \
            --t 1000
...
# take shots
avisar_captura() {                       # grimblast avisa en inglés: el aviso lo damos nosotros
    case "$option_chosen" in
        copy)     notify_user --a "${APP_NAME}" --i "${NOTIFICATION_ICON}" --s "Captura copiada" --m "Lista para pegar" --t 2000 ;;
        save)     notify_user --a "${APP_NAME}" --i "${NOTIFICATION_ICON}" --s "Captura guardada" --m "$screenshot_folder/$NAME" --t 2000 ;;
        copysave) notify_user --a "${APP_NAME}" --i "${NOTIFICATION_ICON}" --s "Captura copiada y guardada" --m "$screenshot_folder/$NAME" --t 2000 ;;
    esac
}

takescreenshot() {
    sleep 1
    grimblast "$option_chosen" "$option_type_screenshot" $NAME && avisar_captura
    if [ -f $HOME/$NAME ]; then
